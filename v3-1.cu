...
/* ---------- CUDA RANDOM ARRAY KERNEL ---------- */




__global__ void init_randarray_k(float* randomNormalArray, curandState* states) {
    
	// Variables for the RNG
	...
	// Optimised by generating random pairs
...
__global__ void MC_k(float dt, float sqrtdt, int steps, float T, int N, float* randomNormalArray, float* results){

    // Determining global thread ID and assigning a random state
    int idx = blockDim.x * blockIdx.x + threadIdx.x, offsetN, offsetG1, offsetG2;

    // Declaring local variables
    float S, Y, price, G1, G2;
    ...
    for (int i = 0; i < N; i++) {
        ...
        S = 1.0f;
        Y = Y0R;
        // Each of the N trajectories has 2000 random values 
        // G1 accesses random values with increasing indexes
		// G1 accesses random values with decreasing indexes
        // Hence the offsets below
...
int main(void) {
	...
	// Declaring and allocating a random array
	// This is completed within seconds ; no time-optimisation required
	// ~1GB GPU memory occupancy ; can be further space-optimised
	...
	for(int i=0; i<16; i++){
		...
		int steps = int(T[i]/dt);
		start_time_mc = clock();
		MC_k<<<GRID_SIZE,BLOCK_SIZE>>>(dt, sqrtdt, steps, T[i], N, randomNormalArray, mcResults);
		cudaDeviceSynchronize();
		end_time_mc = clock();
